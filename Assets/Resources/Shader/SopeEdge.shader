Shader "Custom/ScopeWithDarkEdgeTransparentCenter"
{
    Properties
    {
        ...
        _Darkness ("Edge Darkness", Range(0, 1)) = 1.0 // 외부 어두운 정도 (최대 1로 설정)
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target {
                ...
                if (dist > _ScopeRadius) {
                    // 바깥쪽 영역의 밝기를 더 어둡게 처리 (어두운 색으로 강하게 조정)
                    col.rgb *= (1.0 - _Darkness); // 기존의 어두운 보정값을 크게 변경
                    col.rgb = lerp(col.rgb, float3(0, 0, 0), _Darkness); // 어두운 색으로 보정 (더 어두운 색으로)
                    col.a = 0.5; // 투명도를 적용 (어두운 영역을 반투명으로)
                }
                else {
                    // 원 안쪽은 투명하게 처리
                    col.a = 0.0; // 원 안쪽은 완전히 투명하게 설정
                }
