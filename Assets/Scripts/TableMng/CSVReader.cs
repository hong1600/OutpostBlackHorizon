...
public class CSVReader
{
    ...
    public void getEnum<T>(int row, ref T val) where T : struct, Enum
    {
        string s = _arr_grid[cur_col, row];
        ++cur_col;
        ...
        if (Enum.TryParse(s, true, out T parsedVal))
        {
            val = parsedVal;
        }
        else
    ...
    public void getSprite(int row, ref Sprite val)
    {
        string s = _arr_grid[cur_col, row];
        ...
        if (!string.IsNullOrEmpty(s))
        {
            val = Resources.Load<Sprite>(s);
            if (val == null)
            {
                Debug.LogWarning($"이미지를 찾을 수 없습니다: {s}");
            }
        }
        else
        {
    ...
    public void getScriptableObject<T>(int row, ref T val, Dictionary<int, T> database) where T : UnityEngine.Object
    {
        string s = _arr_grid[cur_col, row];
        ++cur_col;

        if (int.TryParse(s, out int id) && database.TryGetValue(id, out T obj))
        {
            val = obj;
        }
        else
